## ----------------------------------------------------------------------------------------------------------------------------------
## Zybo Z7-20 Xilinx Design Constraints (XDC) File
## ----------------------------------------------------------------------------------------------------------------------------------
## Overview:
## Constraints files map ports in a design to physical hardware. The anatomy of a `set_proprty` command is fairly consistent.
## First, the PACKAGE_PIN is set. We are using the XC7Z020-1CLG400C FPGA SOC on our Zybo Z7-20 development board,
## which has a CL400/CLG400 package. Pins are assigned a "Battleship" style coordinate, A-Y on the y-axis and 1-20 on the x-axis.
## For your reference, you can find the full map in UG865, Zynq-7000 SoC Packaging and Pinout Product Specification, starting on p. 38:
## https://docs.amd.com/v/u/en-US/ug865-Zynq-7000-Pkg-Pinout
## Then, the IOSTANDARD (input/output standard) is set. We are specifying that this board uses the LV CMOS 3.3 V standard.
## Next, we specify which ports in the top-level entity declaration 
## Finally, the schematic (Sch=) connections are listed, for the D.1 Revision of the Zybo board, found here:
## https://files.digilent.com/resources/programmable-logic/zybo-z7/zybo-z7-d1-sch.pdf

## How to use this file:
## - uncomment the lines corresponding to used pins
## - rename the used ports (in each line, after get_ports) according to the top level port names in the project
##	 that is, the ports listed in the entity declaration of your top level file. This entity declaration captures all
## 	 connections to physical IO on the board. If you have components in your project, these are connected internally
##   and thus do not need to be specified in this physical hardware con straints file.

## Compatibility:
## The latest version of the Zybo Z7-20 is Rev. D--however the most recently published .XDC is for the Rev. B version of the board.
## This constraints file can also be used with the Zybo Z7-10 board, and Z7-20 exclusive ports are labeled accordingly.

## -----------------------------------------------------------------------------------------------------------------------------------------------------------
## |NAME			 |PACKAGE PIN #	    |IO STANDARD 		  			  |PORT SPECIFICATION			 									|SCHEMATIC MAPPING|
## |				 |				    |  					  			  | 						     									|		          |																
## -----------------------------------------------------------------------------------------------------------------------------------------------------------
## -----------------------------------------------------------------------------------------------------------------------------------------------------------
## Clock signal
...
## Switches
## -----------------------------------------------------------------------------------------------------------------------------------------------------------
#set_property -dict { PACKAGE_PIN G15   IOSTANDARD LVCMOS33 			} [get_ports { switches_i[0]			}]; #IO_L19N_T3_VREF_35 		Sch=sw[0]
#set_property -dict { PACKAGE_PIN P15   IOSTANDARD LVCMOS33 			} [get_ports { switches_i[1]			}]; #IO_L24P_T3_34 				Sch=sw[1]
#set_property -dict { PACKAGE_PIN W13   IOSTANDARD LVCMOS33 			} [get_ports { switches_i[2]			}]; #IO_L4N_T0_34 				Sch=sw[2]
#set_property -dict { PACKAGE_PIN T16   IOSTANDARD LVCMOS33 			} [get_ports { switches_i[3]			}]; #IO_L9P_T1_DQS_34 			Sch=sw[3]

## -----------------------------------------------------------------------------------------------------------------------------------------------------------
## Buttons
## -----------------------------------------------------------------------------------------------------------------------------------------------------------
#set_property -dict { PACKAGE_PIN K18   IOSTANDARD LVCMOS33 			} [get_ports { buttons_i[0] 		 		}]; #IO_L12N_T1_MRCC_35 		Sch=btn[0]
#set_property -dict { PACKAGE_PIN P16   IOSTANDARD LVCMOS33 			} [get_ports { buttons_i[1] 		 		}]; #IO_L24N_T3_34 				Sch=btn[1]
#set_property -dict { PACKAGE_PIN K19   IOSTANDARD LVCMOS33 			} [get_ports { buttons_i[2] 		 		}]; #IO_L10P_T1_AD11P_35 		Sch=btn[2]
#set_property -dict { PACKAGE_PIN Y16   IOSTANDARD LVCMOS33 			} [get_ports { buttons_i[3] 		 		}]; #IO_L7P_T1_34 				Sch=btn[3]
...
## Pmod Header JC
## -----------------------------------------------------------------------------------------------------------------------------------------------------------                                                                                                                  
#set_property -dict { PACKAGE_PIN V15   IOSTANDARD LVCMOS33 			} [get_ports { dac_lo_mclk_oport 	}]; #IO_L10P_T1_34 				Sch=jc_p[1]   			 
#set_property -dict { PACKAGE_PIN W15   IOSTANDARD LVCMOS33 			} [get_ports { dac_lo_lrclk_oport 	}]; #IO_L10N_T1_34 				Sch=jc_n[1]		     
#set_property -dict { PACKAGE_PIN T11   IOSTANDARD LVCMOS33 			} [get_ports { dac_lo_sclk_oport 	}]; #IO_L1P_T0_34 				Sch=jc_p[2]              
#set_property -dict { PACKAGE_PIN T10   IOSTANDARD LVCMOS33 			} [get_ports { dac_lo_sd_oport 		}]; #IO_L1N_T0_34 				Sch=jc_n[2]              
#set_property -dict { PACKAGE_PIN W14   IOSTANDARD LVCMOS33 			} [get_ports { adc_li_mclk_oport	}]; #IO_L8P_T1_34 				Sch=jc_p[3]              
#set_property -dict { PACKAGE_PIN Y14   IOSTANDARD LVCMOS33 			} [get_ports { adc_li_lrclk_oport 	}]; #IO_L8N_T1_34 				Sch=jc_n[3]              
#set_property -dict { PACKAGE_PIN T12   IOSTANDARD LVCMOS33 			} [get_ports { adc_li_sclk_oport 	}]; #IO_L2P_T0_34 				Sch=jc_p[4]              
#set_property -dict { PACKAGE_PIN U12   IOSTANDARD LVCMOS33 			} [get_ports { adc_li_sd_iport 		}]; #IO_L2N_T0_34 				Sch=jc_n[4]              
...
## Audio Codec
## -----------------------------------------------------------------------------------------------------------------------------------------------------------
set_property -dict { PACKAGE_PIN R19   IOSTANDARD LVCMOS33 } [get_ports { ac_bclk_o }]; #IO_0_34 Sch=ac_bclk
set_property -dict { PACKAGE_PIN R17   IOSTANDARD LVCMOS33 } [get_ports { ac_mclk_o }]; #IO_L19N_T3_VREF_34 Sch=ac_mclk
set_property -dict { PACKAGE_PIN P18   IOSTANDARD LVCMOS33 } [get_ports { ac_mute_n_o }]; #IO_L23N_T3_34 Sch=ac_muten
set_property -dict { PACKAGE_PIN R18   IOSTANDARD LVCMOS33 } [get_ports { ac_dac_data_o }]; #IO_L20N_T3_34 Sch=ac_pbdat
set_property -dict { PACKAGE_PIN T19   IOSTANDARD LVCMOS33 } [get_ports { ac_dac_lrclk_o }]; #IO_25_34 Sch=ac_pblrc
set_property -dict { PACKAGE_PIN R16   IOSTANDARD LVCMOS33 } [get_ports { ac_adc_data_i }]; #IO_L19P_T3_34 Sch=ac_recdat
set_property -dict { PACKAGE_PIN Y18   IOSTANDARD LVCMOS33 } [get_ports { ac_adc_lrclk_o }]; #IO_L17P_T2_34 Sch=ac_reclrc
